...
from routes.legal_services_routes import legal_router
...
def save_uploads(files: list[UploadFile], target_dir: Path) -> list[dict]:
    ...
    for file in files:
        ...
        saved_files.append({
            ...
            "path": str(out_path),
            "url": "/uploads/" + out_path.relative_to(UPLOAD_ROOT).as_posix(),
        })
...
try:
    from services_catalog.catalog import SERVICE_GROUPS
except Exception:
    SERVICE_GROUPS = {}
...
def service_detail(request: Request, service_slug: str):
